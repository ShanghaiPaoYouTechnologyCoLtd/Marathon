...
var requesttimes = [];
var canRequest = true;
var canRequestCache = {}; //是否可以请求缓存列表  //multi 是否允许同时多个请求
function async(url, params, success, fail, method, multi, async) {
	...
	async = async ? true : false; //默认true，开启异步

	if (!multi) {  //同步加载被关闭，进行验证
...
function canRequestAgain(tp) {
	...
	canRequest = canRequestCache["canRequestStatus_" + tp];
 ...
 * @param content  提示内容
 */
function showRequestMessage(status, content, positions, stime) {
	var p = "";
	if (stime == undefined || stime == null || stime.length == 0) {
		stime = 3000;
	}
