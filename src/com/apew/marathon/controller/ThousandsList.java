...
public class ThousandsList extends BaseController {
	...
	public String playerCheck(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(tsService.playerInRace(cardno, raceno)){
			return returnPlayerCheckJson(4, "选手已获取直通车名额", 0);
		}
		int quotas = 0;
		if (listStatus == 1) {
			...
			if (quotas > 0) // 100位有空间
				return returnPlayerCheckJson(0, "获取免费直通车名额", quotas);

			quotas = tsService.getRemainPaidQuotas(raceno);// 没有100位位置，查找500的有没有位置
			if (quotas > 0) // 500有空间
				return returnPlayerCheckJson(1, "获取付费直通车名额", quotas);
